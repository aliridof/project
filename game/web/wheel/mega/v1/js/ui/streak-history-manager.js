...
class StreakHistoryManager {
    ...
    createHistoryGroup(timestamp, entries) {
        ...
        countDiv.textContent = `${entries.length} numbers`;
    ...
    createHistoryItem(entry) {
        ...
        // Add colored bullet based on number
        const bullet = document.createElement('span');
        bullet.className = 'bullet';
        bullet.style.background = Helpers.getNumberColor(parseInt(entry.label));
        labelSpan.appendChild(bullet);
        
        // Add segment info
        const numberInfo = document.createElement('span');
        numberInfo.className = 'number-info';
        numberInfo.textContent = `${entry.label} (${MEGAWHEEL_DATA.segments[parseInt(entry.label)]} segmen)`;
        labelSpan.appendChild(numberInfo);
